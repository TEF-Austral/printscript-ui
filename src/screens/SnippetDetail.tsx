...
export const SnippetDetail = (props: SnippetDetailProps) => {
    ...
    const {
        ...
        mutateAsync: formatSnippetAsync,
    ...
    return (
        <Box p={4} minWidth={'60vw'} data-testid="snippet-detail-container">
            <Box width={'100%'} p={2} display={'flex'} justifyContent={'flex-end'}>
                <CloseIcon style={{cursor: "pointer"}} onClick={handleCloseModal} data-testid="close-detail-button"/>
            </Box>
            {isLoading ? (
                <>
                ...
                <>
                    <Typography variant="h4" fontWeight={"bold"}
                                data-testid="snippet-name">{snippet?.name ?? "Snippet"}</Typography>
                    <Box display="flex" flexDirection="row" gap="8px" padding="8px">
                        <Tooltip title={"Share"}>
                            <IconButton onClick={() => setShareModalOppened(true)} data-testid="share-button">
                                <Share/>
                            </IconButton>
                        </Tooltip>
                        <Tooltip title={"Test"}>
                        ...
                        <Tooltip title={"Download"}>
                            <IconButton onClick={() => setDownloadModalOpen(true)} data-testid="download-button">
                                <Download/>
                            </IconButton>
                        </Tooltip>
                        <Tooltip title={"Load from file"}>
                            <IconButton onClick={() => fileInputRef?.current?.click()} data-testid="upload-button">
                                <Upload/>
                            </IconButton>
                        </Tooltip>
                        <Tooltip title={"Format"}>
                            <IconButton onClick={handleFormat} disabled={isFormatLoading} data-testid="format-button">
                                <ReadMoreIcon/>
                            </IconButton>
                        </Tooltip>
                        <Tooltip title={"Analyze"}>
                            <IconButton onClick={handleAnalyze} disabled={isAnalyzeLoading}
                                        color={analyzeResult?.isValid ? "success" : "default"}
                    ...
                    {analyzeResult && (
                        <Box mb={2} data-testid="analyze-result">
                    ...
                    {testResult && (
                        <Box mb={2} data-testid="test-result">
                            <Alert severity={testResult.passed ? "success" : "error"}>
                                <Typography variant="subtitle2" fontWeight="bold">
                                    Test {testResult.passed ? "Passed ✅" : "Failed ❌"} - Check Output section below for
                                    details
                                </Typography>
                            </Alert>
                        </Box>
                    ...
                    <Box display={"flex"} gap={2}>
                        <SnippetBox flex={1} height={"fit-content"} overflow={"none"} minHeight={"500px"}
                                    bgcolor={'black'} color={'white'} code={code}>
                            <Editor
                                value={code}
                                padding={10}
                    ...
                    <Box pt={1} flex={1} marginTop={2}>
                        ...
                        {testResult ? (
                            <Box mt={2} p={2} bgcolor="background.paper" borderRadius={1} border={1}
                                 borderColor="divider">
